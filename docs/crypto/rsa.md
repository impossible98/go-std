...
This package implements RSA encryption as specified in PKCS #1 and [RFC 8017](https://rfc-editor.org/rfc/rfc8017.html).
...
DecryptPKCS1v15 decrypts a plaintext using RSA and the padding scheme from PKCS #1 v1.5. If random != nil, it uses RSA blinding to avoid timing side-channel attacks.
...
If random is not nil then RSA blinding will be used to avoid timing side-channel attacks.
...
Before encrypting, data is “padded” by embedding it in a known structure. This is done for a number of reasons, but the most obvious is to ensure that the value is large enough that the exponentiation is larger than the modulus.
...
- [notboring.go](/code/crypto/rsa/notboring)
- [pkcs1v15.go](/code/crypto/rsa/pkcs1v15)
- [pss.go](/code/crypto/rsa/pss)
- [rsa.go](/code/crypto/rsa/)
